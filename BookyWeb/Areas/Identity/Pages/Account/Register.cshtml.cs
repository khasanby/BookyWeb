...
namespace BookyWeb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            ReturnUrl = returnUrl;
            var companyResponse = await _companyRepository.GetAllCompanies();
            ExternalLogins = (await _signInManager.GetExternalAuthenticationSchemesAsync()).ToList();
            Input = new InputModel()
            {
                RoleList = _roleManager.Roles.Select(c => c.Name).Select(x => new SelectListItem
                {
                    Text = x,
                    Value = x
                }),
                CompanyList = companyResponse.Data.Select(c => new SelectListItem
                {
                    Text = c.Name,
                    Value = c.Id.ToString()
                })
